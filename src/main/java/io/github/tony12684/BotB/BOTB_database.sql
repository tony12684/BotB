--** = (Foreign key item)
CREATE TABLE IF NOT EXISTS `users` ( --minecraft UUID without dashes
...
ALTER TABLE `games`
  ADD FOREIGN KEY IF NOT EXISTS (`team_id`) REFERENCES `teams`(`team_id`);

ALTER TABLE `user_game_roles`
  ADD FOREIGN KEY IF NOT EXISTS (`role_id`) REFERENCES `roles`(`role_id`),
  ADD FOREIGN KEY IF NOT EXISTS (`game_id`) REFERENCES `games`(`game_id`),
  ADD FOREIGN KEY IF NOT EXISTS (`uuid`) REFERENCES `users`(`uuid`),
  ADD FOREIGN KEY IF NOT EXISTS (`action_id`) REFERENCES `actions`(`action_id`);
